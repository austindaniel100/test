								   ...
								   tKeccakLane * h_outBuffer, tKeccakLane * d_outBuffer)
{
	...
	{
	ker_Keccak<<<NB_THREADS_BLOCKS/NB_STREAMS , NB_THREADS>>>(d_inBuffer + s*inOffset ,d_outBuffer + s*outOffset);
	}//checkCUDAError(" ker_keccak");
								   ...
								   tKeccakLane * h_outBuffer, tKeccakLane * d_outBuffer)
{
	...
	for(s=0;s<NB_STREAMS;s++)
	{
		...
		cudaMemcpyAsync(d_inBuffer + s* inOffset , h_inBuffer + s*inOffset , inOffset * 4 , cudaMemcpyHostToDevice, stream[s]);		
		checkCUDAError(" MemcpyAsync htd");

		ker_Keccak<<<NB_THREADS_BLOCKS/NB_STREAMS , NB_THREADS, stream[s]>>>(d_inBuffer + s*inOffset ,d_outBuffer + s*outOffset);
	}
	checkCUDAError(" ker_keccak");
							 ...
							 tKeccakLane * h_outBuffer, tKeccakLane * d_outBuffer)
{
  ...
  //copy back device to host	
  cudaMemcpy(h_outBuffer,d_outBuffer, 2*OUTPUT_BLOCK_SIZE_B * NB_SCND_STAGE_THREADS * NB_THREADS_BLOCKS ,cudaMemcpyDeviceToHost);
  //checkCUDAError(" Memcpy dth");
}
								   ...
								   tKeccakLane *Kstate)
{
	...
	for(s=0;s<NB_STREAMS;s++)
	{
		...
		ker_Keccak_2stg<<<NB_THREADS_BLOCKS/NB_STREAMS , NB_THREADS, stream[s]>>>(d_inBuffer + s*inOffset ,d_outBuffer + s*outOffset);
	}
								   ...
								   tKeccakLane * h_outBuffer, tKeccakLane * d_outBuffer)
{
	...
	for(s=0;s<NB_STREAMS;s++)
	{
		ker_Keccak_SCipher<<<NB_THREADS_BLOCKS/NB_STREAMS , NB_THREADS, stream[s]>>>(d_inKeyNonce ,d_outBuffer + s*outOffset);
		checkCUDAError(" ker_keccak_SCipher");

		//copy Device to Host, PARTIAL 
		cudaMemcpyAsync(h_outBuffer + s* outOffset , d_outBuffer + s*outOffset ,
