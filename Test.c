...
void TestGPU_Stream()
{
	...
	cudaHostAlloc( (void **)&h_inBuffer, INPUT_BLOCK_SIZE_B * NB_THREADS*NB_THREADS_BLOCKS * NB_INPUT_BLOCK,
	...
	cudaHostAlloc( (void **)&h_outBuffer, OUTPUT_BLOCK_SIZE_B * NB_THREADS * NB_THREADS_BLOCKS,
		cudaHostAllocDefault);
...
void TestGPU_2stg()
{
	...
	//init device outBuffer
	cudaMalloc((void **)&d_outBuffer, 2*OUTPUT_BLOCK_SIZE_B * NB_SCND_STAGE_THREADS * NB_THREADS_BLOCKS );
	checkCUDAError(" cudaMalloc d_outBuffer");
	cudaMemset(d_outBuffer,0, 2*OUTPUT_BLOCK_SIZE_B * NB_SCND_STAGE_THREADS * NB_THREADS_BLOCKS);
	checkCUDAError(" cudaMemset d_outBuffer");
...
void TestGPU_2stg_Stream_OverlapCPU()
{
	...
	//alloc host outBuffer WITH CudaHostAlloc 
	cudaHostAlloc( (void **)&h_outBuffer, 2*OUTPUT_BLOCK_SIZE_B * NB_SCND_STAGE_THREADS * NB_THREADS_BLOCKS,
		cudaHostAllocDefault);
	checkCUDAError("cudaMallocHost h_outBuffer");
...
void TestGPU_SCipher()
{
	...
	//alloc host inBuffer WITH CudaHostAlloc 
	cudaHostAlloc( (void **)&h_inKeyNonce, 2*32 ,
		cudaHostAllocDefault);
	...
	//alloc host outBuffer WITH CudaHostAlloc 
	cudaHostAlloc( (void **)&h_outBuffer, SC_NB_OUTPUT_BLOCK * OUTPUT_BLOCK_SIZE_B * NB_THREADS * NB_THREADS_BLOCKS,
		cudaHostAllocDefault);
	checkCUDAError("cudaMallocHost h_outBuffer");
	...
	//init device outBuffer
	cudaMalloc((void **)&d_outBuffer, SC_NB_OUTPUT_BLOCK * OUTPUT_BLOCK_SIZE_B * NB_THREADS * NB_THREADS_BLOCKS );
	checkCUDAError(" cudaMalloc d_outBuffer");
	cudaMemset(d_outBuffer,0, SC_NB_OUTPUT_BLOCK * OUTPUT_BLOCK_SIZE_B * NB_THREADS * NB_THREADS_BLOCKS);
	checkCUDAError(" cudaMemset d_outBuffer");
...
void Test_Completness()
{
	...
	//free all buffer host and device
	cudaFreeHost(h_inBuffer); cudaFreeHost(h_outBuffer); 	
	cudaFree(d_inBuffer); cudaFree(d_outBuffer);
